    ...
    <CCard>
      <CCardBody>
        <div class="box-title d-flex justify-content-between">
          ...
          <router-link :to="{ name: 'Create Category' }" class="btn btn-success"
            ><b-icon icon="plus"></b-icon>Add New</router-link
          >
        </div>
        <Tables :items="listCategory" :fields="fields" :itemsPerPage="10">
          ...
          <template v-slot:action="{ item }">
            <td class="py-2 pl-2 pr-1 d-flex justify-content-center">
              <CButton
                ...
                color="primary"
                square
                v-on:click="editCategory(item.categoryID)"
                variant="outline"
                size="sm"
              >
              ...
              <CButton
                ...
                color="danger"
                square
                v-on:click="showConfirm(item.categoryID)"
                variant="outline"
                size="sm"
      ...
      <b-modal id="modal-delete" hide-header hide-footer>
        ...
        <div class="d-flex justify-content-center">
          <b-button
            ...
            block
            v-on:click.prevent="deleteCategory(dataModal)"
            >Delete</b-button
          >
          <b-button
...
export default {
  ...
  data() {
    return {
      fields: tableFields.CATEGORY_STORE,
      dataModal: "",
    };
  },
  components: {
  ...
  computed: {
    ...mapGetters(["success", "error", "listCategory", "message"]),
  },
  watch: {
    success() {
      if (this.success) {
  ...
  methods: {
    ...
    ...mapActions({ deleteCategoryByID: "deleteCategoryByID" }),
    showConfirm(id) {
      this.dataModal = id;
