...
export default {
  ...
  watch: {
    listUser() {
      this.optionUser = this.listUser.filter((item) => item.id !== 1);
      this.listUserOptions = this.optionUser.map((item) => ({
        id: item.id.toString(),
        email: item.email,
      }));
      this.listUserOptions.map((item) => {
  ...
  methods: {
    ...mapActions({ createReward: "createReward" }),
